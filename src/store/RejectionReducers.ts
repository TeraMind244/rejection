...
interface IActionMap {
	[actionType: string]: (state: IStateProps, action?: Action<unknown>) => IStateProps;
}
...
const reducer = (state = INITIAL_STATE, action: Action<unknown>): IStateProps => {
	return ACTION_MAP[action.type]?.(state, action) || state;
};

function addQuestion(state: IStateProps, { payload: newQuestion }: Action<IQuestion>): IStateProps {
...
function removeQuestion(state: IStateProps, { payload: id }: Action<string>): IStateProps {
